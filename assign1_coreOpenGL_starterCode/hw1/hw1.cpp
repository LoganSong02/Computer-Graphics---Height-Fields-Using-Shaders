...
void idleFunc()
{
  ...
  if (screenShotsNum >= 301) {
    stop = true;
  }

  if (!stop) {
    if (screenShotsNum <= 50) {
        keyFromUser = 1;
        terrainRotate[1] -= 1.6f;
        terrainRotate[0] += 0.05f;
        terrainRotate[2] += 0.05f;
        
        char filename[1024];
        sprintf(filename, "Screenshots/key_%d_screenshoot_%d_.jpg", keyFromUser, screenShotsNum);

        saveScreenshot(filename);

        screenShotsNum++;
    }
    if (screenShotsNum > 50 && screenShotsNum <= 100) {
        keyFromUser = 2;
        terrainRotate[1] += 1.2f;
        terrainRotate[0] -= 0.05f;
        terrainRotate[2] -= 0.05f;
    ...
    if (screenShotsNum > 100 && screenShotsNum <= 150) {
        keyFromUser = 3;
        terrainRotate[1] -= 1.2f;
        terrainRotate[0] += 0.05f;
        terrainRotate[2] += 0.05f;
    ...
    if (screenShotsNum > 150 && screenShotsNum <= 200) {
      GLint mode = pipelineProgram->GetUniformVariableHandle("mode");
      pipelineProgram->SetUniformVariablei("mode",1);
      GLint scale = pipelineProgram->GetUniformVariableHandle("scale");
      GLint exponent = pipelineProgram->GetUniformVariableHandle("exponent");
      pipelineProgram->SetUniformVariablef("scale",1.0);
      pipelineProgram->SetUniformVariablef("exponent", 1.0);
      keyFromUser = 4;
      terrainRotate[1] += 0.8f;
      terrainRotate[0] -= 0.05f;
      terrainRotate[2] -= 0.05f;
    ...
    if (screenShotsNum > 200 && screenShotsNum <= 225) {
        keyFromUser = 4;

        GLint mode = pipelineProgram->GetUniformVariableHandle("mode");
        pipelineProgram->SetUniformVariablei("mode",1);

        if (screenShotsNum == 201 || screenShotsNum == 215) {
          GLint scale = pipelineProgram->GetUniformVariableHandle("scale");
          float currentScaleValue;

          glGetUniformfv(pipelineProgram->GetProgramHandle(), scale, &currentScaleValue);
          currentScaleValue /= 2.0;
          pipelineProgram->SetUniformVariablef("scale", currentScaleValue);
        }
    ...
    if (screenShotsNum > 225 && screenShotsNum <= 250) {
        keyFromUser = 4;

        GLint mode = pipelineProgram->GetUniformVariableHandle("mode");
        pipelineProgram->SetUniformVariablei("mode",1);

        if (screenShotsNum == 230 || screenShotsNum == 245) {
          GLint scale = pipelineProgram->GetUniformVariableHandle("scale");
          float currentScaleValue;
    ...
    if (screenShotsNum > 250 && screenShotsNum <= 275) {
        keyFromUser = 4;

        GLint mode = pipelineProgram->GetUniformVariableHandle("mode");
        pipelineProgram->SetUniformVariablei("mode",1);

        if (screenShotsNum == 255 || screenShotsNum == 270) {
          GLint scale = pipelineProgram->GetUniformVariableHandle("exponent");
          float currentScaleValue;

          glGetUniformfv(pipelineProgram->GetProgramHandle(), scale, &currentScaleValue);
          currentScaleValue *= 2.0;
          pipelineProgram->SetUniformVariablef("exponent", currentScaleValue);
        }
    ...
    if (screenShotsNum > 275 && screenShotsNum <= 300) {
        keyFromUser = 4;

        GLint mode = pipelineProgram->GetUniformVariableHandle("mode");
        pipelineProgram->SetUniformVariablei("mode",1);

        if (screenShotsNum == 280 || screenShotsNum == 290) {
          GLint scale = pipelineProgram->GetUniformVariableHandle("exponent");
          float currentScaleValue;
...
{
  // GLint mode = glGetUniformLocation(pipelineProgram->GetProgramHandle(), "mode");
  // GLint scale = glGetUniformLocation(pipelineProgram->GetProgramHandle(), "scale");
  // GLint exponent = glGetUniformLocation(pipelineProgram->GetProgramHandle(), "exponent");
  ...
  switch (key)
  {
    ...
    case '9':
      glGetUniformfv(pipelineProgram->GetProgramHandle(), exponent, &currentScaleValue);
      currentScaleValue *= 2.0;
      pipelineProgram->SetUniformVariablef("exponent", currentScaleValue);
    break;
...
void initKey1() {
  ...
  numVertices = Width * height; // This must be a global variable, so that we know how many vertices to render in glDrawArrays.
  ...
  for (int i = 0; i < Width; i++) {
    for (int j = 0; j < height; j++) {
      int position = (i * height + j) * 3;
      int colorPos = (i * height + j) * 4;
      positions[position] = 1.0 * i / (height - 1);
      positions[position+1] = 0.2 * heightmapImage->getPixel(i,j,0) / 255.0f;
      positions[position+2] = 1.0 * (-j) / (height - 1);
      colors[colorPos] = 1.0f * heightmapImage->getPixel(i,j,0) / 255.0f;
      colors[colorPos+1] = 1.0f * heightmapImage->getPixel(i,j,0) / 255.0f;
      colors[colorPos+2] = 1.0f * heightmapImage->getPixel(i,j,0) / 255.0f;
      colors[colorPos+3] = 1.0f;
    }
  }
...
void initkey2() {
  ...
  numVertices = (Width-1) * height * 2 + (height-1) * Width * 2; // This must be a global variable, so that we know how many vertices to render in glDrawArrays.
  ...
  for (int i = 0; i < height; i++) {
    for (int j = 0; j < Width-1; j++) {
      int position = pointSoFar * 3;
      int colorPos = colorSofar * 4;
      positions[position] = 1.0 * i / (height - 1);
      positions[position+1] = 0.2 * heightmapImage->getPixel(i,j,0) / 255.0f;
      positions[position+2] = 1.0 * (-j) / (height - 1);

      positions[position+3] = 1.0 * (i) / (height - 1);
      positions[position+4] = 0.2 * heightmapImage->getPixel(i,j+1,0) / 255.0f;
      positions[position+5] = 1.0 * (-(j+1)) / (height - 1);

      colors[colorPos] = 1.0 * heightmapImage->getPixel(i,j,0) / 255.0f;
      colors[colorPos+1] = 1.0 * heightmapImage->getPixel(i,j,0) / 255.0f;
      colors[colorPos+2] = 1.0 * heightmapImage->getPixel(i,j,0) / 255.0f;
      colors[colorPos+3] = 1.0;
      colors[colorPos+4] = 1.0 * heightmapImage->getPixel(i,j+1,0) / 255.0f;
      colors[colorPos+5] = 1.0 * heightmapImage->getPixel(i,j+1,0) / 255.0f;
      colors[colorPos+6] = 1.0 * heightmapImage->getPixel(i,j+1,0) / 255.0f;
      colors[colorPos+7] = 1.0;

      pointSoFar = pointSoFar + 2;
      colorSofar = colorSofar + 2;
    }
  }
  ...
  for (int j = 0; j < Width; j++) {
    for (int i = 0; i < height-1; i++) {
      ...
      positions[position+3] = 1.0 * (i+1) / (height - 1);
      positions[position+4] = 0.2 * heightmapImage->getPixel(i+1,j,0) / 255.0f;
      positions[position+5] = 1.0 * (-j) / (height - 1);

      colors[colorPos] = 1.0 * heightmapImage->getPixel(i,j,0) / 255.0f;
      colors[colorPos+1] = 1.0 * heightmapImage->getPixel(i,j,0) / 255.0f;
      colors[colorPos+2] = 1.0 * heightmapImage->getPixel(i,j,0) / 255.0f;
      colors[colorPos+3] = 1.0;
      colors[colorPos+4] = 1.0 * heightmapImage->getPixel(i+1,j,0) / 255.0f;
      colors[colorPos+5] = 1.0 * heightmapImage->getPixel(i+1,j,0) / 255.0f;
      colors[colorPos+6] = 1.0 * heightmapImage->getPixel(i+1,j,0) / 255.0f;
      colors[colorPos+7] = 1.0;
